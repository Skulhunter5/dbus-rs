...
fn main() {
    let dbus_session_bus_address = std::env::var("DBUS_SESSION_BUS_ADDRESS").expect("DBUS_SESSION_BUS_ADDRESS is not set");
    ...
    let connection = Connection::init(path).expect("failed to initialize connection to dbus session bus");
